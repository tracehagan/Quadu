...
void QuaduinoFlight::init() {
  ...
  minPulseWidth = 0;
  maxPulseWidth = 255;
  gravityHover = 50; //proportional to voltage of battery... :(
  ...
  frontMotorSteering = -4;
  rearMotorSteering = -4;
  rightMotorSteering = 4;
  leftMotorSteering = 4;
...
void QuaduinoFlight::setupPidControl() {
  ...
  rollSetpoint = 89; // level
  pitchSetpoint = 182; // level
...
boolean QuaduinoFlight::checkOrientation(double pitchIn, double rollIn) {
  double pitchInput;
  if(pitchIn < 360/2) {
      pitchInput = map(pitchIn, 0, 180, 180, 0);
  } else if(pitchIn > 360/2) {
  ...
  if (pitchInput <= 90 || pitchInput >= 270 || rollIn >= 180){
      return true;
  } else {
      return false;
  } 
